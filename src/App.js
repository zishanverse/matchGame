...
class App extends Component {
  constructor(props) {
    super(props)
    this.state = {score: 0, time: 60, tabId: 'FRUIT', result: false}
    this.ran = 0
    this.timerId = 0
  }
  ...
  changeResult = () => {
    this.setState({result: false, score: 0, time: 60})
  ...
  rendering = result => {
    ...
    const list = imagesList.filter(each => each.category === tabId)
    if (result) {
      return (
    ...
    return (
      <div className="container">
        <img
          src={imagesList[this.ran].imageUrl}
          alt="match"
          className="match"
        />
        <ul className="tabCard">
          <li className="tab">
            <button
              type="button"
              className={`tabName ${tabId === 'FRUIT' ? 'select' : ''}`}
              onClick={this.changeTabF}
            >
              {tabsList[0].displayText}
            </button>
            <hr className={`line ${tabId === 'FRUIT' ? 'selectLine' : ''}`} />
          </li>

          <li className="tab">
            <button
              type="button"
              onClick={this.changeTabA}
              className={`tabName ${tabId === 'ANIMAL' ? 'select' : ''}`}
            >
              {tabsList[1].displayText}
            </button>
            <hr className={`line ${tabId === 'ANIMAL' ? 'selectLine' : ''}`} />
          </li>

          <li className="tab">
            <button
              type="button"
              onClick={this.changeTabP}
              className={`tabName ${tabId === 'PLACE' ? 'select' : ''}`}
            >
              {tabsList[2].displayText}
            </button>
            <hr className={`line ${tabId === 'PLACE' ? 'selectLine' : ''}`} />
          </li>
        </ul>
        ...
        <div>
          <ul className="listCard">
            {list.map(each => (
              <Item key={each.id} item={each} func={this.matching} />
            ))}
          </ul>
        </div>
      </div>
  ...
  changeTabF = () => {
    this.setState({tabId: 'FRUIT'})
  }

  changeTabA = () => {
    this.setState({tabId: 'ANIMAL'})
  }

  changeTabP = () => {
    this.setState({tabId: 'PLACE'})
  }

  matching = id => {
    const imageId = imagesList[this.ran].id

    if (id === imageId) {
      this.setState(pre => ({score: pre.score + 1}))
      this.ran = Math.ceil(Math.random() * 30)
  ...
  render() {
    const {score, time, result} = this.state

    return (
      <>
        <Navbar scr={score} tm={time} />
